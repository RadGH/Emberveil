import{z as ut,B as dt,D as mt,E as gt,F as G,H as bt,I as K,J as ft,O as W,Q as Y,R as ht,G as _,q,i as vt,c as kt,U as Z,V as J,p as xt,k as yt,t as $t,v as Q,S as V,W as tt,X as et,C as _t,Y as j,N as st,Z as wt,r as at}from"./play-CVwbNx0X.js";import{showConfirmModal as it}from"./ConfirmModal-BOjaCEqK.js";import"./modulepreload-polyfill-B5Qt9EMX.js";let I=null;function nt(){I&&I.parentNode&&I.parentNode.removeChild(I),I=null}function St(p,e){const l=e.getBoundingClientRect(),t=p.getBoundingClientRect();let d=l.left+l.width/2-t.width/2,s=l.top-t.height-10;d<10&&(d=10),d+t.width>window.innerWidth-10&&(d=window.innerWidth-t.width-10),s<10&&(s=l.bottom+10),p.style.left=`${d}px`,p.style.top=`${s}px`}function ot(p,e,l={}){if(!p)return()=>{};const t=l.className||"rsg-tooltip",d=g=>{nt();const f=e();if(!f)return;const b=document.createElement("div");b.className=t,b.innerHTML=f,document.body.appendChild(b),I=b,St(b,g.currentTarget||p)},s=()=>nt();p.addEventListener("mouseenter",d),p.addEventListener("mouseleave",s);let r=null;const i=g=>{clearTimeout(r),r=setTimeout(()=>d(g),350)},o=()=>{clearTimeout(r),setTimeout(s,1500)},a=()=>{clearTimeout(r),s()};return p.addEventListener("touchstart",i,{passive:!0}),p.addEventListener("touchend",o),p.addEventListener("touchcancel",a),()=>{p.removeEventListener("mouseenter",d),p.removeEventListener("mouseleave",s),p.removeEventListener("touchstart",i),p.removeEventListener("touchend",o),p.removeEventListener("touchcancel",a),clearTimeout(r)}}function lt(){if(document.getElementById("rsg-tooltip-styles"))return;const p=document.createElement("style");p.id="rsg-tooltip-styles",p.textContent=`
    .rsg-tooltip {
      position: fixed;
      z-index: 3000;
  ...
  `,document.head.appendChild(p)}function Pt(p){return p==="magic"?"Magic Damage":p==="light"?"Light Damage":"Heavy Damage"}function Ct(p,e={}){const{withSpendButtons:l=!1,pendingAttrPoints:t=0,includeHeader:d=!1,headerLabel:s=l?"Attributes":"Character Stats",baseToggleId:r="stats-base-chk"}=e;if(!p)return'<div class="stat-row"><span>No character selected</span></div>';const i=p.baseAttrs||p.attrs||{STR:8,DEX:8,INT:8,CON:8},o=p.attrs||i,a=ut(),g=p.equipment||{};let f=0;for(const c of Object.values(g))c!=null&&c.armor&&(f+=c.armor);const b=0,m=dt(p);f+=m.armor||0;const h=mt(g),v=a?i:{STR:(o.STR||8)+(m.str||0),DEX:(o.DEX||8)+(m.dex||0),INT:(o.INT||8)+(m.int||0),CON:(o.CON||8)+(m.con||0)},$=gt(g)+(m.dmg||0),n=G(v,a?0:$,h),u=G(i,0,h),x=Pt(h),M=a?0:m.magicResist||0,T=(c,k)=>({hp:k?Y(p):50+c.CON*10,mp:k?W(p):30+c.INT*8,hit:Math.min(95,70+Math.round(c.DEX*1.2)+(k&&m.hit||0)),dodge:Math.min(40,5+Math.round(c.DEX*.8)+(k&&m.dodge||0)),spl:+(c.INT*.025+(k&&m.spellPower||0)).toFixed(2)}),y=T(a?i:v,!a),E=T(i,!1),S=a?b:f,U=a?0:bt(p).resistAll||0,z=K(S,U),R=K(b,0),w=(c,k,A)=>{const B=ht(k,A,c);return B?` style="color:${B}"`:""},L=["STR","DEX","INT","CON"].map(c=>{const k=c.toLowerCase(),A=v[c],B=w(k,A,i[c]);if(l){const X=t>0;return`
    ...
    <div class="stat-row" title="Armor ${(z.armorDr*100).toFixed(1)}% + Misc ${(z.miscDr*100).toFixed(1)}% (multiplicative)"><span class="sr-label stat-label" data-stat="Damage Reduction">Damage Reduction</span><span class="sr-val"${w("dmgReduction",z.totalDr,R.totalDr)}>${(z.totalDr*100).toFixed(1)}%</span></div>
