const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./StatsScreen-DHA1Bp37.js","./play-B4Rs_XUc.js","./savesClient-Lt_9u8Ks.js","./play-dErdlDnR.css"])))=>i.map(i=>d[i]);
import{r as A,i as S,c as L,F as M,G as m,H as $,I as _,p as E,_ as C,P as H,J as P,K as N,L as z,N as I}from"./play-B4Rs_XUc.js";import"./savesClient-Lt_9u8Ks.js";function f(i){return i?!!i.isCompanion||i.class==="companion":!1}function X(i){return!f(i)}const q=`
.party-screen {
...
`,T={tank:{label:"Tank",color:"#4080c0",svg:'<rect x="3" y="7" width="8" height="7" rx="1"/><path d="M7 3L3 7h8z"/>'},healer:{label:"Healer",color:"#40c080",svg:'<path d="M7 3v8M3 7h8"/>'},support:{label:"Support",color:"#c8a020",svg:'<path d="M7 2l1.5 4H13l-3.5 2.5 1.3 4L7 10 3.2 12.5l1.3-4L1 6h4.5z"/>'},dps:{label:"DPS",color:"#c04040",svg:'<path d="M3 11L11 3M9 3h2v2"/><path d="M3 11l3-1 1-3"/>'}};function R(i){if(!i.class||i.class==="companion"||i.isCompanion)return null;const e=I.find(s=>s.id===i.class),t=(e==null?void 0:e.role)||"";return/tank|front|wall|fortress/i.test(t)?"tank":/heal|cleric|medic|restor/i.test(t)?"healer":/support|buff|maestro|bard|shaman/i.test(t)?"support":"dps"}function B(i){const e=R(i);if(!e)return"";const t=T[e];return`<span class="ps-role-badge" title="${t.label}" aria-label="Role: ${t.label}" style="--rb-color:${t.color}"><svg viewBox="0 0 14 14" width="14" height="14" fill="${t.color}" stroke="${t.color}" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${t.svg}</svg></span>${P("role_badges")}`}function D(i){typeof i.hp=="number"&&typeof i.maxHp=="number"&&i.hp>i.maxHp&&(i.hp=i.maxHp);const e=Math.round((i.hp||0)/(i.maxHp||1)*100),t=e>60?"#60c060":e>30?"#e8c840":"#e06040",s=typeof i.hp=="number"?Math.floor(i.hp):i.hp??"?",n=typeof i.maxHp=="number"?Math.floor(i.maxHp):i.maxHp??"?";return`HP: <span style="color:${t}">${s}/${n}</span>`}function j(i){if(!i||i.isCompanion||i.class==="companion")return"";const e=i.level||1,t=i.xp||0,s=N(e),n=z(e);if(n==null)return`<div class="ps-xp-row"><span class="ps-xp-label">XP:</span><div class="ps-xp-bar" title="MAX (Level ${e})"><div class="ps-xp-fill" style="width:100%"></div></div></div>`;const o=Math.max(1,n-s),l=Math.max(0,t-s),r=Math.max(0,Math.min(100,Math.round(l/o*100)));return`<div class="ps-xp-row"><span class="ps-xp-label">XP:</span><div class="ps-xp-bar" title="${`XP: ${t} / ${n} (Level ${e} → ${e+1})`}"><div class="ps-xp-fill" style="width:${r}%"></div></div></div>`}class O{constructor(e,t){this.manager=e,this.audio=t,this._el=null,this._selectedA=null}onEnter(){this._build()}onPause(){this._el&&(this._el.style.display="none")}onResume(){this._el&&(this._el.style.display="flex")}onExit(){A(this._el),this._el=null}_build(){S("party-screen-styles",q),this._el=L("div","party-screen"),this.manager.uiOverlay.appendChild(this._el),this._render()}_render(){return M(this._el,()=>this._renderImpl())}_renderImpl(){var r;const e=m.get(),t=e.bench||[],s=t.filter(a=>!f(a)),n=t.filter(a=>f(a)),o=`
      <div class="ps-col">
      ...
      <div class="ps-col">
        ...
        <div class="ps-slots">
          ${n.length>0?n.map(a=>this._slotHTML(a,"bench",t.indexOf(a))).join(""):'<div class="ps-hint">No inactive companions.</div>'}
        </div>
    ...
    `,this._wire()}_detailHTML(){const e=this._getSelected();if(!e)return"";const t=e.defaultName||e.name,s=e.customName&&e.customName!==t?`<span class="ps-default-name" title="Default name (cleared if rename is left blank)">orig: ${t}</span>`:"";return`
      <div class="ps-detail">
        <div class="ps-detail-title">
          ...
          ${$(e,14,"ps-slot-class-icon")} — ${e.className||e.class} Lv.${e.level||1}
          ${s}
        </div>
      ...
      </div>`}_commitRename(e){var n,o;const t=this._getSelected();if(!t||!e)return;const s=(e.value||"").trim();t.defaultName||(t.defaultName=t.name),s?s===t.defaultName?(t.customName="",t.name=t.defaultName):(t.customName=s,t.name=s):(t.customName="",t.name=t.defaultName);try{(o=(n=_).saveCurrentGame)==null||o.call(n)}catch{}this._render()}_wireRename(){const e=this._el;if(!e)return;const t=e.querySelector("#ps-rename-display"),s=e.querySelector("#ps-rename-input"),n=e.querySelector("#ps-detail-rename");if(!t||!s)return;const o=()=>{t.hidden=!0,s.hidden=!1,s.focus(),s.select()},l=()=>{s.hidden=!0,t.hidden=!1};t.addEventListener("click",o),t.addEventListener("keydown",r=>{(r.key==="Enter"||r.key===" ")&&(r.preventDefault(),o())}),n==null||n.addEventListener("click",()=>{var r,a;(a=(r=this.audio)==null?void 0:r.playSfx)==null||a.call(r,"click"),o()}),s.addEventListener("keydown",r=>{r.key==="Enter"?(r.preventDefault(),this._commitRename(s)):r.key==="Escape"&&(r.preventDefault(),l())}),s.addEventListener("blur",()=>{this._commitRename(s)})}_slotHTML(e,t,s){const n=this._selectedA&&this._selectedA.source===t&&this._selectedA.index===s,o=t==="bench"?"reserve":t==="companions"?"companion":"party",l=m.get(),r=t==="party"&&l.party.length>1||t==="companions",a=e.isCompanion||e.class==="companion",u=t==="bench"&&(a?l.companions.length<4:l.party.length<4);return`<div class="ps-slot${n?" ps-selected":""}" data-source="${t}" data-index="${s}">
      <div class="ps-slot-icon">${E(e,32)}</div>
      <div class="ps-slot-info">
        <div class="ps-slot-name">${e.name}${e.isPet&&e.ownerName?` (${e.ownerName}'s pet)`:""} ${$(e,14,"ps-slot-class-icon")}${B(e)}</div>
    ...
    </div>`}_wire(){var o,l,r,a,u;this._el.querySelector("#ps-close").addEventListener("click",()=>{this.audio.playSfx("click"),this.manager.pop()}),(o=this._el.querySelector("#ps-stats"))==null||o.addEventListener("click",async()=>{this.audio.playSfx("click");const{StatsScreen:c}=await C(async()=>{const{StatsScreen:d}=await import("./StatsScreen-DHA1Bp37.js");return{StatsScreen:d}},__vite__mapDeps([0,1,2,3]),import.meta.url);this.manager.push(new c(this.manager,this.audio,{tab:"party"}))}),this._wireRename(),this._el.querySelectorAll(".ps-slot[data-source]").forEach(c=>{c.addEventListener("click",()=>{const d=c.dataset.source,p=parseInt(c.dataset.index);this._onSlotClick(d,p)})}),this._el.querySelectorAll("[data-dismiss]").forEach(c=>{c.addEventListener("click",d=>{d.stopPropagation();const[p,b]=c.dataset.dismiss.split(":"),g=parseInt(b),h=m.get(),y=p==="party"?h.party:h.companions;if(p==="party"&&h.party.length<=1)return;const[x]=y.splice(g,1);if(h.bench=h.bench||[],h.bench.push(x),!x.isCompanion&&x.id){const w=v=>v.isPet&&v.ownerId===x.id,k=h.companions.filter(w);h.companions=h.companions.filter(v=>!w(v));for(const v of k)h.bench.push(v)}this._selectedA=null,this.audio.playSfx("click"),_.saveCurrentGame(),this._render()})}),this._el.querySelectorAll("[data-recruit]").forEach(c=>{c.addEventListener("click",d=>{d.stopPropagation();const p=parseInt(c.dataset.recruit),b=m.get(),[g]=b.bench.splice(p,1);if(g.isCompanion||g.class==="companion"){if(b.companions.length>=4)return;b.companions.push(g)}else{if(b.party.length>=4)return;b.party.push(g)}this._selectedA=null,this.audio.playSfx("click"),_.saveCurrentGame(),this._render()})});const e=c=>{const d=this._getSelected();if(!d)return;this.audio.playSfx("click");const p=m.get(),b=p.party.includes(d)||p.companions.includes(d);p.inventoryContext=b?"default":"party-inactive",p.inventoryFocusId=d.id;const g=[...p.party||[],...p.companions||[]],h=Math.max(0,g.findIndex(y=>y&&y.id===d.id));this.manager.replace(new H(this.manager,this.audio,{tab:c,charIdx:h}))};(l=this._el.querySelector("#ps-detail-inv"))==null||l.addEventListener("click",()=>e("inventory")),(r=this._el.querySelector("#ps-detail-spells"))==null||r.addEventListener("click",()=>e("spells")),(a=this._el.querySelector("#ps-detail-passives"))==null||a.addEventListener("click",()=>e("passives")),(u=this._el.querySelector("#ps-detail-attrs"))==null||u.addEventListener("click",()=>e("attributes"));const t=this._el.querySelector("#ps-swap");t&&!t.disabled&&t.addEventListener("click",()=>this._doSwap());const s=this._el.querySelector("#ps-move-up");s&&!s.disabled&&s.addEventListener("click",()=>this._doMove(-1));const n=this._el.querySelector("#ps-move-down");n&&!n.disabled&&n.addEventListener("click",()=>this._doMove(1))}_onSlotClick(e,t){if(!this._selectedA)this._selectedA={source:e,index:t};else if(this._selectedA.source===e&&this._selectedA.index===t)this._selectedA=null;else{this._doSwapWith({source:e,index:t});return}this._render()}_getSelected(){if(!this._selectedA)return null;const e=m.get();return(this._selectedA.source==="party"?e.party:this._selectedA.source==="companions"?e.companions:e.bench)[this._selectedA.index]}_canSwap(){return this._selectedA!==null}_canMoveUp(){return this._selectedA&&this._selectedA.source==="party"&&this._selectedA.index>0}_canMoveDown(){const e=m.get();return this._selectedA&&this._selectedA.source==="party"&&this._selectedA.index<e.party.length-1}_doSwap(){const e=m.get();if(!this._selectedA||e.bench.length===0)return;const t=this._selectedA.source==="party"?e.party:e.companions,s=t[this._selectedA.index],n=e.bench[0];t[this._selectedA.index]=n,e.bench[0]=s,e.bench.splice(0,1,s),this._selectedA=null,this._autosave(),this._render()}_doSwapWith(e){const t=m.get(),s=u=>u==="party"?t.party:u==="companions"?t.companions:t.bench,n=s(this._selectedA.source),o=s(e.source),l=n[this._selectedA.index],r=o[e.index];if(this._selectedA.source==="companions"&&r&&!f(r)){this._selectedA=null,this._render();return}if(e.source==="companions"&&l&&!f(l)){this._selectedA=null,this._render();return}if(this._selectedA.source==="party"&&r&&f(r)){this._selectedA=null,this._render();return}if(e.source==="party"&&l&&f(l)){this._selectedA=null,this._render();return}const a=n[this._selectedA.index];n[this._selectedA.index]=o[e.index],o[e.index]=a,t.party=t.party.filter(Boolean),t.companions=t.companions.filter(Boolean),this._selectedA=null,this._autosave(),this._render()}_autosave(){try{_.saveCurrentGame()}catch{}}_doMove(e){const t=m.get();if(!this._selectedA||this._selectedA.source!=="party")return;const s=this._selectedA.index,n=s+e;n<0||n>=t.party.length||([t.party[s],t.party[n]]=[t.party[n],t.party[s]],this._selectedA.index=n,this._render())}}export{O as PartyScreen,f as isCompanionEntity,X as isHeroEntity};
