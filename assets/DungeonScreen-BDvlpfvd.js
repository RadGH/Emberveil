const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/play-CVwbNx0X.js","assets/modulepreload-polyfill-B5Qt9EMX.js","assets/play-dErdlDnR.css"])))=>i.map(i=>d[i]);
import{ah as L,i as N,G as o,r as m,c as x,ai as k,aj as S,a9 as U,ab as P,_ as T,ak as V}from"./play-CVwbNx0X.js";import"./modulepreload-polyfill-B5Qt9EMX.js";const I=1.35,E=1.3,$=4,w=6,q=`
...
`;class A{constructor(e,t,s,i,a){this.manager=e,this.audio=t,this._dungeonId=s,this._anchorNodeId=i,this._anchorZoneId=a,this._dungeon=L[s],this._stageIdx=0,this._el=null,this._pendingStunBuff=!1,this._victoryPending=!1}onEnter(){if(N("dg-styles",q),!this._dungeon){console.warn("[DungeonScreen] unknown dungeonId",this._dungeonId),this.manager.pop();return}this._build()}onResume(){if(this._el&&(this._el.style.display=""),this._victoryPending){this._victoryPending=!1,this._showVictoryCard();return}if(this._nextStagePending){if(this._nextStagePending=!1,this._stageIdx++,this._stageIdx>=this._dungeon.stages.length){this._doVictorySequence();return}this._build();return}if(this._lastCombatVictory){this._lastCombatVictory=!1;const e=o.get();if([...e.party||[],...e.companions||[]].some(s=>s&&s._lastLevelUp)){this._nextStagePending=!0;return}if(this._stageIdx++,this._stageIdx>=this._dungeon.stages.length){this._doVictorySequence();return}}else if(this._lastCombatDefeat){this._lastCombatDefeat=!1,this.manager.pop();return}this._build()}onLeave(){this._purgeDom()}onExit(){this._purgeDom()}destroy(){this._purgeDom()}_purgeDom(){try{m(this._el)}catch{}try{document.querySelectorAll(".dg-screen").forEach(e=>e.remove())}catch{}this._el=null}_build(){if(this._el&&m(this._el),this._el=x("div","dg-screen"),this._stageIdx>=this._dungeon.stages.length){this._doVictorySequence();return}const e=this._dungeon.stages[this._stageIdx],t=this._dungeon.stages.length,s=Array.from({length:t},(r,d)=>d<this._stageIdx?'<div class="pip done"></div>':d===this._stageIdx?'<div class="pip active"></div>':'<div class="pip"></div>').join(""),i=e.type,a=`Stage ${this._stageIdx+1} / ${t}`;let n="";if(i==="combat"||i==="boss"){const r=k[e.encounter],g=this._scaledEncounterEnemies(r).reduce((h,u)=>h+(u.count||1),0),l=i==="boss",p=this._pendingStunBuff?'<div class="dg-buff-badge">Enemies will start stunned (skill check bonus)</div>':"";n=`
        ...
        </div>`}else if(i==="skill_check"){const r=S[e.checkId]||{};n=`
        <div class="dg-card">
          ...
          <p>${r.flavor||"Something blocks your path."}</p>
          <p style="font-size:0.78rem;color:#a090c0">
            Roll ${r.stat||"?"} vs DC ${r.dc||"?"} &mdash;
            Pass: enemies start stunned next fight. Fail: party takes ~${Math.round((r.failDamagePct||.12)*100)}% max HP damage.
          </p>
          <div class="dg-buttons">
            <button type="button" class="dg-btn danger" id="dg-giveup">Give Up</button>
            <button type="button" class="dg-btn primary" id="dg-attempt">Attempt (${r.stat||"?"} ${r.dc||"?"})</button>
          </div>
    ...
    `,this.manager.uiOverlay.appendChild(this._el),this._wire()}_wire(){const e=this._el.querySelector("#dg-giveup");e&&e.addEventListener("click",()=>this._giveUp());const t=this._el.querySelector("#dg-fight");t&&t.addEventListener("click",()=>this._engageCombat());const s=this._el.querySelector("#dg-attempt");s&&s.addEventListener("click",()=>this._resolveSkillCheck())}_scaledEncounterEnemies(e){if(!e)return[];const s=(e.enemies||[]).map(n=>({...n})).map(n=>({...n,hp:Math.round((n.hp||1)*I),maxHp:Math.round((n.maxHp||n.hp||1)*I),dmg:Array.isArray(n.dmg)?[Math.round(n.dmg[0]*E),Math.round(n.dmg[1]*E)]:n.dmg})),i=s.reduce((n,r)=>n+(r.count||1),0);if(i<$){const n=$-i;s.length>0&&(s[s.length-1]={...s[s.length-1],count:(s[s.length-1].count||1)+n})}const a=s.reduce((n,r)=>n+(r.count||1),0);if(a<w&&s.length>0){const n=w-a;s[0]={...s[0],count:(s[0].count||1)+n}}return s}_engageCombat(){var a,n;const e=this._dungeon.stages[this._stageIdx],t=k[e.encounter];if(!t){console.warn("[DungeonScreen] missing encounter",e.encounter),this._advance();return}const s=this._scaledEncounterEnemies(t),i={...t,enemies:s,name:e.name||t.name,_zoneId:this._anchorZoneId,_dungeonStage:this._stageIdx};if(this._pendingStunBuff){this._pendingStunBuff=!1;const r=o.get();r._pendingDungeonStunRound1=!0}(n=(a=this.audio)==null?void 0:a.playSfx)==null||n.call(a,"click"),this._lastCombatVictory=!0,this._lastCombatDefeat=!1,this._el&&(this._el.style.display="none"),this.manager.push(new U(this.manager,this.audio,null,i))}_resolveSkillCheck(){var u,b,_,f,v,y;(b=(u=this.audio)==null?void 0:u.playSfx)==null||b.call(u,"click");const e=this._dungeon.stages[this._stageIdx],t=S[e.checkId]||{},s=o.get(),i=(_=s.party)==null?void 0:_[0],a=t.stat||"STR",n=t.dc||12,r=((f=i==null?void 0:i.attrs)==null?void 0:f[a])||8,d=Math.floor(Math.random()*20)+1,g=r+d,l=g>=n;let p="";if(l)this._pendingStunBuff=!0,p=`
        <div class="dg-skill-result pass">
          <strong>Success!</strong> Rolled ${d} + ${r} (${a}) = ${g} vs DC ${n}.<br>
          ${t.passText||"You succeed."}
        </div>`;else{const C=t.failDamagePct||.12,D=[...s.party||[],...s.companions||[]].filter(c=>c&&c.hp>0);for(const c of D){const M=Math.max(1,Math.round((c.maxHp||c.hp||1)*C));c.hp=Math.max(1,(c.hp||1)-M)}p=`
        <div class="dg-skill-result fail">
          <strong>Failure.</strong> Rolled ${d} + ${r} (${a}) = ${g} vs DC ${n}.<br>
          ${t.failText||"You fail."}
