import{E as w,a3 as m,a4 as $,Y as p,a5 as b,a6 as k,a7 as C,J as f,a8 as h,a9 as I,k as g,aa as S,ab as T,ac as E,V as z,ad as L,X as _,ae as v,af as D,ag as y,ah as M,ai as x}from"./play-QH26u79V.js";import"./savesClient-Lt_9u8Ks-B0TWHWS2.js";const A=`
.id-screen {
...
`;class j{constructor(e,t){this.manager=e,this.audio=t,this._el=null,this._run=null,this._stages=[],this._stageIdx=0,this._floorEffects=[],this._floorCleared=!1,this._lastLoot=null,this._timerInterval=null}onEnter(){w("id-styles",A),this._run=m(),this._run||(this._run=$()),this._prepareFloor(),this._build(),this._startTimer()}onResume(){this._el&&(this._el.style.display=""),this._pendingCombatResult==="victory"?(this._pendingCombatResult=null,this._onNodeCleared()):this._build()}onPause(){this._el&&(this._el.style.display="none")}onExit(){this._stopTimer(),p(this._el),this._el=null}destroy(){this._stopTimer(),this._pendingCombatResult==="victory"&&b(!1),p(this._el),this._el=null}_prepareFloor(){if(!this._run)return;const e=this._run.floor;this._floorEffects=k(e,this._run.seed||0),this._stages=C(e,this._floorEffects,this._run.seed||0),this._stageIdx=0,this._floorCleared=!1,this._lastLoot=null}_build(){this._el&&p(this._el),this._el=f("div","id-screen");const e=this._run;if(!e){this.manager.pop();return}const t=e.floor,r=t%5===0,i=Math.round((Date.now()-(e.runStartedAt||Date.now()))/1e3),d=this._floorEffects.map(a=>`<span class="id-effect-chip" style="color:${a.color};border-color:${a.color}40;background:${a.color}15" title="${a.desc}">${a.icon} ${a.name}</span>`).join(""),n=this._floorEffects.length?`<div class="id-effects-bar"><span class="id-effects-label">Floor effects</span>${d}</div>`:"",c=this._stages.length,s=Array.from({length:c},(a,u)=>u<this._stageIdx?'<div class="id-pip done"></div>':u===this._stageIdx?'<div class="id-pip active"></div>':'<div class="id-pip"></div>').join("");let o="";this._floorCleared?o=this._renderFloorClearedCard(t):o=this._renderStageCard(t,r);const l=this._lastLoot?`<div class="id-loot-result">${this._lastLoot}</div>`:"";this._el.innerHTML=`
    ...
    `,this.manager.uiOverlay.appendChild(this._el),this._wireEvents()}_renderStageCard(e,t){if(this._stageIdx>=this._stages.length)return"";const r=this._stages[this._stageIdx],i=`Node ${this._stageIdx+1} / ${this._stages.length}`;if(r.type==="combat")return`
        ...
        </div>`;if(r.type==="anchor_boss"){const d=x(e);return`
      ...
      </div>`}_wireEvents(){var e,t,r,i;(e=this._el.querySelector("#id-btn-fight"))==null||e.addEventListener("click",()=>{this.audio.playSfx("click"),this._engageCombat()}),(t=this._el.querySelector("#id-btn-loot"))==null||t.addEventListener("click",()=>{this.audio.playSfx("click"),this._claimTreasure()}),(r=this._el.querySelector("#id-btn-descend"))==null||r.addEventListener("click",()=>{this.audio.playSfx("click"),I(),this._run=m(),this._prepareFloor(),this._build()}),(i=this._el.querySelector("#id-btn-town"))==null||i.addEventListener("click",()=>{this.audio.playSfx("click"),this._showReturnConfirm()})}_engageCombat(){const e=this._run;if(!e)return;const t=e.floor,r=this._stages[this._stageIdx],i=g.get(),d=Math.round((i.party||[]).reduce((o,l)=>o+(l.level||1),0)/Math.max(1,(i.party||[]).length));let n,c=!1;if(r.type==="anchor_boss"){c=!0;const o=x(t);n=[{...S(o,t,d),count:1}]}else n=T(t,this._floorEffects,e.seed||0).map(o=>E(o,t,d));const s={id:`infinite_floor${t}_node${this._stageIdx}`,name:c?`Anchor Boss — Floor ${t}`:`Floor ${t} — Node ${this._stageIdx+1}`,enemies:n,_zoneId:"infinite_depths",_infiniteFloor:t,_isAnchorBoss:c,_floorEffects:this._floorEffects,isBoss:c};this._el&&(this._el.style.display="none"),this._pendingCombatResult="victory",this.manager.push(new z(this.manager,this.audio,null,s))}_onNodeCleared(){const e=this._run;if(!e)return;const t=this._stages[this._stageIdx],r=L(e.floor,this._getLootMods()),i=.45+e.floor*.01,d=this._floorEffects.some(s=>s.id==="double_damage"),n=[],c=d?2:1;for(let s=0;s<c;s++)if(Math.random()<i*r.dropChanceMult){const o=["sword","dagger","ring","necklace","medium_chest","heavy_chest","staff","wand","cloth_chest","heavy_helm"],l=o[Math.floor(Math.random()*o.length)],a=_(l,r.rarity,r.quality);a&&(g.addToInventory(a),v(a),n.push(a))}if(t.type==="anchor_boss"){D(e.floor);const s=["ring","necklace","heavy_chest","staff","sword"],o=s[Math.floor(Math.random()*s.length)],l=_(o,"legendary","exotic");l&&(g.addToInventory(l),v(l),n.push(l))}this._lastLoot=n.length?`Loot: ${n.map(s=>`<span style="color:var(--rarity-${s.rarity})">${s.name}</span>`).join(", ")}`:"No loot dropped this node.",this._stageIdx++,this._stageIdx>=this._stages.length&&(this._floorCleared=!0),this._el&&(this._el.style.display=""),this._build()}_getLootMods(){const e={};for(const t of this._floorEffects)t.lootMod&&Object.assign(e,t.lootMod);return e}_claimTreasure(){this._pendingCombatResult=null,this._onNodeCleared()}_showRunEndSummary(){var e,t,r;const i=this._run?Math.round((Date.now()-(this._run.runStartedAt||Date.now()))/1e3):0,d=((e=this._run)==null?void 0:e.floor)||1,n=(((t=this._run)==null?void 0:t.runLoot)||[]).length;this._el&&p(this._el),this._el=f("div","id-screen"),this._el.innerHTML=`
      <div class="id-header">
        <div class="id-header-title">Run Ended</div>
        <span class="id-floor-badge">Floor ${d}</span>
      </div>
      <div class="id-body">
        <div class="id-run-summary">
          <h3>The Depths Claimed You</h3>
          <div class="id-stat-row"><span>Deepest floor</span><strong>${d}</strong></div>
          <div class="id-stat-row"><span>Items claimed</span><strong>${n}</strong></div>
          <div class="id-stat-row"><span>Run time</span><strong>${h(i)}</strong></div>
    ...
    `,this.manager.uiOverlay.appendChild(this._el),(r=this._el.querySelector("#id-btn-town-end"))==null||r.addEventListener("click",()=>{this.audio.playSfx("click"),this._returnToTown()})}_returnToTown(){var e;this._run&&b(!1),this.manager.pop(),((e=this.manager._stack)==null?void 0:e[this.manager._stack.length-1])instanceof y||this.manager.push(new y(this.manager,this.audio,null))}_showReturnConfirm(){const e=f("div","id-confirm-overlay");e.innerHTML=`
    ...
    `,document.body.appendChild(e),e.querySelector("#id-cancel-town").addEventListener("click",()=>e.remove()),e.querySelector("#id-confirm-town").addEventListener("click",()=>{e.remove(),this.audio.playSfx("click"),this._returnToTown()})}_renderLeaderboardSnippet(){const e=M().slice(0,5);return e.length?`
      <div class="id-run-summary" style="max-width:460px;width:100%">
        <div class="id-lb-title">Top Runs</div>
        ${e.map((t,r)=>`<div class="id-lb-row">
        <span class="id-lb-floor">#${r+1} F${t.deepestFloor}</span>
        <strong>${t.heroName}</strong>
        <span>${t.heroClass}</span>
        <span>${h(t.runTime)}</span>
      </div>`).join("")}
      </div>`:""}_startTimer(){this._stopTimer(),this._timerInterval=setInterval(()=>{var e;const t=(e=this._el)==null?void 0:e.querySelector("#id-timer");if(!t||!this._run)return;const r=Math.round((Date.now()-(this._run.runStartedAt||Date.now()))/1e3);t.textContent=h(r)},1e3)}_stopTimer(){this._timerInterval&&(clearInterval(this._timerInterval),this._timerInterval=null)}}export{j as InfiniteDungeonScreen};
