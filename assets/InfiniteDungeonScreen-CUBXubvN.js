import{i as w,j as g,k,r as p,l as b,n as $,o as C,c as f,q as h,t as I,G as m,v as E,w as S,x as T,C as L,y as F,z as _,B as y,D as z,T as v,E as R,F as x}from"./play-CctEuBOF.js";import"./modulepreload-polyfill-B5Qt9EMX.js";const A=`
.id-screen {
...
`;class B{constructor(e,t){this.manager=e,this.audio=t,this._el=null,this._run=null,this._stages=[],this._stageIdx=0,this._floorEffects=[],this._floorCleared=!1,this._lastLoot=null,this._timerInterval=null}onEnter(){w("id-styles",A),this._run=g(),this._run||(this._run=k()),this._prepareFloor(),this._build(),this._startTimer()}onResume(){this._el&&(this._el.style.display=""),this._pendingCombatResult==="victory"?(this._pendingCombatResult=null,this._onNodeCleared()):this._build()}onPause(){this._el&&(this._el.style.display="none")}onExit(){this._stopTimer(),p(this._el),this._el=null}destroy(){this._stopTimer(),this._pendingCombatResult==="victory"&&b(!1),p(this._el),this._el=null}_prepareFloor(){if(!this._run)return;const e=this._run.floor;this._floorEffects=$(e,this._run.seed||0),this._stages=C(e,this._floorEffects,this._run.seed||0),this._stageIdx=0,this._floorCleared=!1,this._lastLoot=null}_build(){this._el&&p(this._el),this._el=f("div","id-screen");const e=this._run;if(!e){this.manager.pop();return}const t=e.floor,r=t%5===0,o=Math.round((Date.now()-(e.runStartedAt||Date.now()))/1e3),l=this._floorEffects.map(d=>`<span class="id-effect-chip" style="color:${d.color};border-color:${d.color}40;background:${d.color}15" title="${d.desc}">${d.icon} ${d.name}</span>`).join(""),i=this._floorEffects.length?`<div class="id-effects-bar"><span class="id-effects-label">Floor effects</span>${l}</div>`:"",c=this._stages.length,s=Array.from({length:c},(d,u)=>u<this._stageIdx?'<div class="id-pip done"></div>':u===this._stageIdx?'<div class="id-pip active"></div>':'<div class="id-pip"></div>').join("");let n="";this._floorCleared?n=this._renderFloorClearedCard(t):n=this._renderStageCard(t,r);const a=this._lastLoot?`<div class="id-loot-result">${this._lastLoot}</div>`:"";this._el.innerHTML=`
    ...
    `,this.manager.uiOverlay.appendChild(this._el),this._wireEvents()}_renderStageCard(e,t){if(this._stageIdx>=this._stages.length)return"";const r=this._stages[this._stageIdx],o=`Node ${this._stageIdx+1} / ${this._stages.length}`;if(r.type==="combat")return`
      ...
      </div>`}_wireEvents(){var e,t,r,o;(e=this._el.querySelector("#id-btn-fight"))==null||e.addEventListener("click",()=>{this.audio.playSfx("click"),this._engageCombat()}),(t=this._el.querySelector("#id-btn-loot"))==null||t.addEventListener("click",()=>{this.audio.playSfx("click"),this._claimTreasure()}),(r=this._el.querySelector("#id-btn-descend"))==null||r.addEventListener("click",()=>{this.audio.playSfx("click"),I(),this._run=g(),this._prepareFloor(),this._build()}),(o=this._el.querySelector("#id-btn-town"))==null||o.addEventListener("click",()=>{this.audio.playSfx("click"),this._showReturnConfirm()})}_engageCombat(){const e=this._run;if(!e)return;const t=e.floor,r=this._stages[this._stageIdx],o=m.get(),l=Math.round((o.party||[]).reduce((n,a)=>n+(a.level||1),0)/Math.max(1,(o.party||[]).length));let i,c=!1;if(r.type==="anchor_boss"){c=!0;const n=x(t);i=[{...E(n,t,l),count:1}]}else i=S(t,this._floorEffects,e.seed||0).map(a=>T(a,t,l));const s={id:`infinite_floor${t}_node${this._stageIdx}`,name:c?`Anchor Boss — Floor ${t}`:`Floor ${t} — Node ${this._stageIdx+1}`,enemies:i,_zoneId:"infinite_depths",_infiniteFloor:t,_isAnchorBoss:c,_floorEffects:this._floorEffects,isBoss:c};this._el&&(this._el.style.display="none"),this._pendingCombatResult="victory",this.manager.push(new L(this.manager,this.audio,null,s))}_onNodeCleared(){const e=this._run;if(!e)return;const t=this._stages[this._stageIdx],r=F(e.floor,this._getLootMods()),o=.45+e.floor*.01,l=this._floorEffects.some(s=>s.id==="double_damage"),i=[],c=l?2:1;for(let s=0;s<c;s++)if(Math.random()<o*r.dropChanceMult){const n=["sword","dagger","ring","necklace","medium_chest","heavy_chest","staff","wand","cloth_chest","heavy_helm"],a=n[Math.floor(Math.random()*n.length)],d=_(a,r.rarity,r.quality);d&&(m.addToInventory(d),y(d),i.push(d))}if(t.type==="anchor_boss"){z(e.floor);const s=["ring","necklace","heavy_chest","staff","sword"],n=s[Math.floor(Math.random()*s.length)],a=_(n,"legendary","exotic");a&&(m.addToInventory(a),y(a),i.push(a))}this._lastLoot=i.length?`Loot: ${i.map(s=>`<span style="color:var(--rarity-${s.rarity})">${s.name}</span>`).join(", ")}`:"No loot dropped this node.",this._stageIdx++,this._stageIdx>=this._stages.length&&(this._floorCleared=!0),this._el&&(this._el.style.display=""),this._build()}_getLootMods(){const e={};for(const t of this._floorEffects)t.lootMod&&Object.assign(e,t.lootMod);return e}_claimTreasure(){this._pendingCombatResult=null,this._onNodeCleared()}_showRunEndSummary(){var o,l,i;const e=this._run?Math.round((Date.now()-(this._run.runStartedAt||Date.now()))/1e3):0,t=((o=this._run)==null?void 0:o.floor)||1,r=(((l=this._run)==null?void 0:l.runLoot)||[]).length;this._el&&p(this._el),this._el=f("div","id-screen"),this._el.innerHTML=`
      <div class="id-header">
